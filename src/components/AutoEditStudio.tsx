...
export default function AutoEditStudio({ onBack }: Props) {
  ...
  const [brief, setBrief] = useState<AutoEditBrief>({ prompt: '', style: 'cinematic', targetDuration: 30, format: 'youtube', objectFit: 'contain' });
  ...
  const [generationSeconds, setGenerationSeconds] = useState(0); const generationStarted = useRef(0);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setGenerationSeconds((performance.now() - generationStarted.current) / 1000), 100);
  ...
  const addFiles = async (files: File[]) => {
    ...
    try {
      ...
      setClips((current) => [...current, ...analyzed]); setShots([]); setPreviewUrl(''); setGenerationSeconds(0);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Clip analysis failed.'); }
    finally { setAnalyzing(false); }
  };

  const generate = async () => {
    const next = createStoryline(clips, brief); setShots(next); setSelectedShotId(next[0]?.id || ''); generationStarted.current = performance.now(); setGenerationSeconds(0); setGenerating(true); setGenerateProgress(0); setError('');
    try { const blob = await processAutoEditSequence(clips, next, brief.format, setGenerateProgress, true, brief.objectFit); if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(URL.createObjectURL(blob)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not build the generated preview.'); }
    finally { setGenerationSeconds((performance.now() - generationStarted.current) / 1000); setGenerating(false); }
  };
  ...
  const removeClip = (id: string) => { const clip = clipMap.get(id); if (clip) URL.revokeObjectURL(clip.url); setClips((current) => current.filter((item) => item.id !== id)); setShots((current) => current.filter((shot) => shot.clipId !== id)); setPreviewUrl(''); setGenerationSeconds(0); };
  const exportFirstCut = async () => {
    if (!shots.length) return; setExporting(true); setError(''); setExportProgress(0);
    try { const blob = await processAutoEditSequence(clips, shots, brief.format, setExportProgress, false, brief.objectFit); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `cinemaster-first-cut-${Date.now()}.mp4`; anchor.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); }
  ...
  return <main className="auto-edit-page auto-classic">
    <section className="auto-classic-stage">
      <div className={`auto-classic-canvas ${brief.format === 'instagram' ? 'portrait-output' : 'landscape-output'}`} ref={previewRef}>
        {generating ? <div className="auto-rendering"><LoaderCircle className="spin" /><strong>Building your AI first cut</strong><span>Assembling the proposed movie… {Math.round(generateProgress * 100)}% · {generationSeconds.toFixed(1)}s</span><i><b style={{ width: `${generateProgress * 100}%` }} /></i></div>
          : previewUrl || selectedClip ? <><video ref={videoRef} key={previewUrl || selectedClip?.id} src={previewUrl || selectedClip?.url} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onClick={togglePlayback} /><button className="auto-preview-play" onClick={togglePlayback}>{playing ? <Pause /> : <Play />}</button><button className="auto-preview-fullscreen" onClick={() => void previewRef.current?.requestFullscreen()}><Expand /> Fullscreen</button>{previewUrl && <div className="auto-generated-badge"><Sparkles /> AI first cut · {time(totalDuration)} · generated in {generationSeconds.toFixed(1)}s</div>}</>
    ...
    <aside className="auto-classic-controls">
      ...
      <div className="auto-control-section"><div className="auto-control-title simple"><strong>Output & duration</strong></div><div className="auto-format-row"><button className={brief.format === 'youtube' ? 'active' : ''} onClick={() => { setBrief({ ...brief, format: 'youtube' }); setPreviewUrl(''); setGenerationSeconds(0); }}><Play /> 16:9</button><button className={brief.format === 'instagram' ? 'active' : ''} onClick={() => { setBrief({ ...brief, format: 'instagram' }); setPreviewUrl(''); setGenerationSeconds(0); }}>▯ 9:16</button></div><div className="auto-format-row"><button className={brief.objectFit === 'cover' ? 'active' : ''} onClick={() => { setBrief({ ...brief, objectFit: 'cover' }); setPreviewUrl(''); setGenerationSeconds(0); }}>Fill frame</button><button className={brief.objectFit === 'contain' ? 'active' : ''} onClick={() => { setBrief({ ...brief, objectFit: 'contain' }); setPreviewUrl(''); setGenerationSeconds(0); }}>Fit entire clip</button></div><div className="auto-duration-options">{[15, 30, 60, 90].map((value) => <button key={value} className={brief.targetDuration === value ? 'active' : ''} onClick={() => { setBrief({ ...brief, targetDuration: value }); setPreviewUrl(''); setGenerationSeconds(0); }}>{value}s</button>)}</div>{generationSeconds > 0 && !generating && <div className="auto-generation-time"><Check /> Last preview generated in <strong>{generationSeconds.toFixed(1)} seconds</strong></div>}</div>
